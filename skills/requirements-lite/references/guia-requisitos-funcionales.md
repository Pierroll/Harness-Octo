...
|----------------------|----------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------|
| **ID**               | Identificador único, con el módulo adentro: `RF-<MÓDULO>-<NNN>`.                 | `RF-APRO-003`                                                                                |
...
| **Dependencias**     | Requisitos u otros componentes necesarios para su cumplimiento.                 | `Depende del RF-AUTH-001: Registro de usuario autenticado`                                   |
...
El módulo va dentro del identificador porque el catálogo se lee por módulo y se construye
por módulo. `RF-APRO-003` dice de qué habla sin abrir el documento, sobrevive a que se
inserte un requisito en medio, y es lo que después permite amarrar cada tarea del
cronograma al requisito que implementa (`templates/implementation-schedule.md`). Los no
funcionales usan la categoría de calidad en el mismo lugar: `RNF-SEG-002`.
